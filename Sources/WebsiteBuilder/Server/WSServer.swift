...
// TODO replace Apple's code
    ...
    let websocketResponse = """
        ...
        <html>
          <head>
            ...
            <script>
                var wsconnection = new WebSocket("ws://localhost:\(_port)/websocket");
    ...
    struct WSServer {
        ...
        func run() async throws {
            let channel:
                NIOAsyncChannel<EventLoopFuture<UpgradeResult>, Never> =
                    ...
                    group.addTask {
                        ...
                        while true {
                            ...
                            var buffer = channel.channel.allocator.buffer(
                                capacity: 12)
                            buffer.writeString(
                                "\(channel.channel.remoteAddress)::\(theTime)")
        ...
        private func handleHTTPChannel(
            ...
            try await channel.executeThenClose { inbound, outbound in
                for try await requestPart in inbound {
                    ...
                    headers.add(
                        name: "Content-Length",
                        value: String(Self.responseBody.readableBytes))
